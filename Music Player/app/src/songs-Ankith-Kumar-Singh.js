...
export const getSuggestedSongs = async () => {
  ...
  //   if (!response.ok) {
  //     console.eroor( new Error(`Error fetching songs: ${response.statusText}`));
  //   }
...
export const searchSongs = async (query) => {
  try {
    ...
    if (!response.ok) {
      console.error( new Error(`Error searching for songs: ${response.statusText}`));
    }
...
export const getLyrics = async (songId) => {
  try {
    ...
    if (!response.ok) {
      console.error(new Error(`Error fetching lyrics: ${response.statusText}`));
    }
